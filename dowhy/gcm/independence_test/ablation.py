""" Ablation based (conditional) independence test. Testing independence via regression, i.e. if a variable has
...
def ablation_based(X: np.ndarray,
                   Y: np.ndarray,
    ...
    Z. In the pairwise case (Z is not given), the performances between predicting Y based on X and predicting Y by
    returning its mean (the best estimator without any inputs) are compared.

    Here, we use the Nystroem approach to approximate a kernel map of the inputs that serves as new input features.
    ...
    Note: As compared to kernel_based, this method is quite fast and provides reasonably well results. However, there
    are types of dependencies that this test can not detect. For instance, if X determines the variance of Y, then this
    can not be captured. For these more complex dependencies, consider using the kernel_based independence test instead.
    ...
    :param num_components_all_inputs: Number of kernel features when combining X and Z. Half of the number is used when
                                      compared with Z alone. If Z is not given, Z will be ignored.
    ...
    :return: The p-value for the null hypothesis that X and Y are independent given Z. If not Z is given,
    ...
    for _ in range(num_runs):
        ...
        all_p_values.append(estimate_f_test_p_value(all_inputs[training_indices],
