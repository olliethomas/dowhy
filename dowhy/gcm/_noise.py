...
def get_noise_dependent_function(causal_model: StructuralCausalModel,
    ...
    """Returns a function that represent the given target_node and that is only dependent on upstream noise nodes.
    ...
    :param approx_prediction_model: Prediction model for approximating f. The model is trained based on drawn noise and
                                    target samples.
    :param num_training_samples: Number of drawn samples for training the predictor based on approx_predictor_factory.
                                 Note: This parameter is ignored if approx_predictor_factory is None.
