...
from dowhy.gcm import auto, fit, kernel_based, validate_pnl_assumptions, \
    reject_causal_structure, RejectionResult, InvertibleStructuralCausalModel
...
@flaky(max_runs=5)
def test_validate_causal_graph_collider():
    # collider: X->Z<-Y
    ...
    rejection_result, rejection_summary = reject_causal_structure(collider_dag, data)
...
@flaky(max_runs=5)
def test_validate_causal_graph_chain():
    # chain: X->Z->Y
    ...
    data = pd.DataFrame(data=dict(X=X, Y=Y, Z=Z))
    rejection_result, rejection_summary = reject_causal_structure(chain_dag, data)
...
@flaky(max_runs=5)
def test_validate_causal_graph_fork():
    # fork: X<-Z->Y
    ...
    Y = 3 * Z + np.random.normal(size=500)
    data = pd.DataFrame(data=dict(X=X, Y=Y, Z=Z))
    rejection_result, rejection_summary = reject_causal_structure(fork_dag, data)
...
@flaky(max_runs=5)
def test_validate_causal_graph_general():
    ...
    data = pd.DataFrame(data=dict(X=X, Y=Y, Z=Z))
    rejection_result, rejection_summary = reject_causal_structure(general_dag, data)
...
@flaky(max_runs=5)
def test_validate_causal_graph_adjusted_p_values():
    # fork: X<-Z->Y
    fork_dag = nx.DiGraph([('Z', 'X'), ('Z', 'Y')])
    Z = np.random.normal(size=500)
    X = 2 * Z + np.random.normal(size=500)
    Y = 3 * Z + np.random.normal(size=500)
    data = pd.DataFrame(data=dict(X=X, Y=Y, Z=Z))
    rejection_result, rejection_summary = reject_causal_structure(fork_dag, data, fdr_control_method='fdr_bh')
...
def test_when_using_validate_causal_graph_without_fdrc_then_nans_for_adjusted_p_values_are_returned():
    fork_dag = nx.DiGraph([('Z', 'X'), ('Z', 'Y')])
    Z = np.random.normal(size=500)
    X = 2 * Z + np.random.normal(size=500)
    Y = 3 * Z + np.random.normal(size=500)
    data = pd.DataFrame(data=dict(X=X, Y=Y, Z=Z))
    _, rejection_summary = reject_causal_structure(fork_dag, data, fdr_control_method=None)
...
@flaky(max_runs=2)
def test_given_non_linear_data_and_correct_dag_when_validate_pnl_assumptions_then_returns_true():
    ...
    assert validate_pnl_assumptions(causal_model,
                                    data,
                                    independence_test=lambda x, y: kernel_based(x, y, bootstrap_num_runs=5))


@flaky(max_runs=2)
def test_given_non_linear_data_and_incorrect_dag_when_validate_pnl_assumptions_then_returns_false():
    data = _generate_simple_non_linear_data()
    ...
    assert not validate_pnl_assumptions(causal_model,
                                        data,
                                        independence_test=lambda x, y: kernel_based(x, y, bootstrap_num_runs=5))


@flaky(max_runs=3)
def test_given_non_linear_data_and_incorrect_dag_with_collider_when_validate_pnl_assumptions_then_returns_false():
    data = _generate_simple_non_linear_data()
    ...
    assert not validate_pnl_assumptions(causal_model,
                                        data,
                                        independence_test=lambda x, y: kernel_based(x, y, bootstrap_num_runs=100))
