...
from dowhy.gcm.feature import feature_relevance, feature_relevance_sample, feature_relevance_distribution
...
@flaky(max_runs=5)
def test_when_using_feature_relevance_with_continous_data_then_returns_correct_results():
    ...
    relevance, noise = feature_relevance(causal_model, 'X2')
...
@flaky(max_runs=5)
def test_when_using_feature_relevance_with_categorical_data_then_returns_correct_results():
    ...
    relevance, noise = feature_relevance(causal_model, 'Y', num_background_samples=1000, num_evaluation_samples=100)
...
@flaky(max_runs=5)
def test_when_using_feature_relevance_with_confidence_intervals_then_returns_reasonable_bounds():
    causal_model = StructuralCausalModel(nx.DiGraph([('X1', 'X2'), ('X0', 'X2')]))
    causal_model.set_causal_mechanism('X1', ScipyDistribution(stats.norm, loc=0, scale=1))
    causal_model.set_causal_mechanism('X0', ScipyDistribution(stats.norm, loc=0, scale=1))
    causal_model.set_causal_mechanism('X2', AdditiveNoiseModel(prediction_model=create_linear_regressor()))
    ...
    def estimation_func():
        dict_result, noise = feature_relevance(causal_model, 'X2')
...
def test_feature_relevance_sample_mean_diff():
    ...
    shapley_values = feature_relevance_sample(model,
                                              ...
                                              feature_attribution_func=means_difference)
...
def test_given_baseline_values_when_estimating_feature_relevance_sample_with_mean_diff_then_returns_expected_result():
    ...
    shapley_values_1 = \
        feature_relevance_sample(model,
                                 feature_samples=X,
                                 samples_of_interest=X[:20],
                                 feature_attribution_func=lambda x, y: np.mean(x) * y,
...
@flaky(max_runs=3)
def test_given_misspecified_graph_when_estimating_feature_relevance_with_observed_datas_then_returns_correct_result():
    ...
    feature_relevance_missing_edge, _ = feature_relevance(causal_model_without, 'X2', training_data)
    feature_relevance_with_edge, _ = feature_relevance(causal_model_with, 'X2')
